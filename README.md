...
`Button`'s primary variant originally used `--accent` (`#0090d9`) as its idle background with white text — only ~3.5:1 contrast, failing WCAG AA for normal text (4.5:1 required). Its idle background now uses the darker `--accent-hover` (`#007ebd`, ~4.6:1, passes AA), and a new `--accent-active` (`#005f8f`) was added for the hover ripple fill so hover still reads as a distinct, deeper state.
